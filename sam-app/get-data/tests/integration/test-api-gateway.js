...
describe("Test API Gateway", function () {
   ...
   * here we use cloudformation API to find out what the GetSongDataApi URL is
   */
  before(async () => {
    ...
    const apiOutput = stackOutputs.find(
      (output) => output.OutputKey === "GetSongDataApi"
    );

    expect(apiOutput, `Cannot find output GetSongDataApi in stack ${stackName}`)
      .not.to.be.undefined;
